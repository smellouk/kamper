...
actual fun KonitorState.initialize(scope: CoroutineScope) {
    val ctx = checkNotNull(appContext) { "appContext must be set before initializing Konitor" }
    ...
    Konitor.install(MemoryModule(ctx))
    Konitor.install(NetworkModule)
    Konitor.install(IssuesModule(ctx, anr = AnrConfig(thresholdMs = 5_000L)) {
        crash { chainToPreviousHandler = false }
    })
    Konitor.install(JankModule(app))
    Konitor.install(GcModule)
    Konitor.install(ThermalModule(ctx))
    ...
    Konitor.addInfoListener<JankInfo>    { info -> if (info != JankInfo.INVALID && info != JankInfo.UNSUPPORTED) scope.launch { jankInfo = info } }
