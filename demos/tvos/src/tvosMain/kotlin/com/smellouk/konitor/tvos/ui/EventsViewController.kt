...
class EventsViewController : UIViewController(nibName = null, bundle = null) {
    ...
    override fun viewDidLoad() {
        ...
        loginTarget      = ActionTarget { Konitor.logEvent("user_login") }
        purchaseTarget   = ActionTarget { Konitor.logEvent("purchase") }
        screenViewTarget = ActionTarget { Konitor.logEvent("screen_view") }
        ...
        customInput = UITextField().apply {
            ...
            font = UIFont.monospacedSystemFontOfSize(28.0, weight = 0.0)
        ...
        c += footerScroll.heightAnchor.constraintEqualToConstant(180.0)

        c += outerStack.topAnchor.constraintEqualToAnchor(footerScroll.topAnchor, constant = 20.0)
        c += outerStack.leadingAnchor.constraintEqualToAnchor(footerScroll.leadingAnchor, constant = 32.0)
        c += outerStack.trailingAnchor.constraintEqualToAnchor(footerScroll.trailingAnchor, constant = -32.0)
        c += outerStack.bottomAnchor.constraintEqualToAnchor(footerScroll.bottomAnchor, constant = -20.0)
...
private fun eventRowView(event: UserEventInfo, wallMs: Long): UIView {
    ...
    val nameLabel = UILabel().apply {
        text = event.name
        textColor = Theme.TEXT
        font = UIFont.monospacedSystemFontOfSize(24.0, weight = UIFontWeightMedium)
        translatesAutoresizingMaskIntoConstraints = false
    }
    ...
    val c = mutableListOf<NSLayoutConstraint>()
    c += container.heightAnchor.constraintGreaterThanOrEqualToConstant(80.0)
    ...
    c += nameLabel.leadingAnchor.constraintEqualToAnchor(bar.trailingAnchor, constant = 20.0)
    c += nameLabel.trailingAnchor.constraintEqualToAnchor(timeLabel.leadingAnchor, constant = -16.0)
    ...
    event.durationMs?.let { dur ->
        ...
        c += nameLabel.topAnchor.constraintEqualToAnchor(container.topAnchor, constant = 16.0)
        c += durLabel.leadingAnchor.constraintEqualToAnchor(bar.trailingAnchor, constant = 20.0)
        c += durLabel.topAnchor.constraintEqualToAnchor(nameLabel.bottomAnchor, constant = 4.0)
        c += durLabel.bottomAnchor.constraintEqualToAnchor(container.bottomAnchor, constant = -16.0)
    } ?: run {
