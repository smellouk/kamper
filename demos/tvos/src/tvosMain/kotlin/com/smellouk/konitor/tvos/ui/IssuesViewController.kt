...
class IssuesViewController : UIViewController(nibName = null, bundle = null) {
    ...
    override fun viewDidLoad() {
        ...
        c += btnStack.topAnchor.constraintEqualToAnchor(footerScroll.topAnchor, constant = 20.0)
        c += btnStack.leadingAnchor.constraintEqualToAnchor(footerScroll.leadingAnchor, constant = 32.0)
        c += btnStack.trailingAnchor.constraintEqualToAnchor(footerScroll.trailingAnchor, constant = -32.0)
        c += btnStack.bottomAnchor.constraintEqualToAnchor(footerScroll.bottomAnchor, constant = -20.0)
        c += btnStack.heightAnchor.constraintEqualToAnchor(footerScroll.heightAnchor, constant = -40.0)
    ...
    private fun triggerSlowSpan() {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            IssueSpans.measure("tvos-demo-op", thresholdMs = 300L) {
                usleep(800_000u)
            }
        }
    }
    ...
    private fun triggerCrash() {
        ...
        addIssue(Issue(
            ...
            message = "Demo crash from K|tvOS",
            timestampMs = ts
        ))
    }
}
...
private fun issueRowView(issue: Issue): UIView {
    ...
    val typeChip = UILabel().apply {
        ...
        font = UIFont.monospacedSystemFontOfSize(20.0, weight = UIFontWeightBold)
    ...
    val timeLabel = UILabel().apply {
        text = fmtTimeTvos(issue.timestampMs)
        textColor = Theme.MUTED
        font = UIFont.monospacedSystemFontOfSize(22.0, weight = 0.0)
        translatesAutoresizingMaskIntoConstraints = false
    }
    ...
    val details = buildDetailsTvos(issue)
    var detailLabel: UILabel? = null
    if (details.isNotEmpty()) {
        detailLabel = UILabel().apply {
            text = details
            textColor = Theme.MUTED
            font = UIFont.monospacedSystemFontOfSize(20.0, weight = 0.0)
            translatesAutoresizingMaskIntoConstraints = false
        }
    ...
    val c = mutableListOf<NSLayoutConstraint>()
    c += container.heightAnchor.constraintGreaterThanOrEqualToConstant(100.0)
    ...
    c += bar.widthAnchor.constraintEqualToConstant(8.0)

    c += typeChip.leadingAnchor.constraintEqualToAnchor(bar.trailingAnchor, constant = 20.0)
    c += typeChip.topAnchor.constraintEqualToAnchor(container.topAnchor, constant = 16.0)

    c += sevLabel.leadingAnchor.constraintEqualToAnchor(typeChip.trailingAnchor, constant = 12.0)
    c += sevLabel.centerYAnchor.constraintEqualToAnchor(typeChip.centerYAnchor)

    c += timeLabel.trailingAnchor.constraintEqualToAnchor(container.trailingAnchor, constant = -20.0)
    c += timeLabel.centerYAnchor.constraintEqualToAnchor(typeChip.centerYAnchor)

    c += msgLabel.leadingAnchor.constraintEqualToAnchor(bar.trailingAnchor, constant = 20.0)
    c += msgLabel.trailingAnchor.constraintEqualToAnchor(container.trailingAnchor, constant = -20.0)
    c += msgLabel.topAnchor.constraintEqualToAnchor(typeChip.bottomAnchor, constant = 8.0)

    if (detailLabel != null) {
        c += detailLabel.leadingAnchor.constraintEqualToAnchor(bar.trailingAnchor, constant = 20.0)
        c += detailLabel.trailingAnchor.constraintEqualToAnchor(container.trailingAnchor, constant = -20.0)
        c += detailLabel.topAnchor.constraintEqualToAnchor(msgLabel.bottomAnchor, constant = 4.0)
        c += detailLabel.bottomAnchor.constraintEqualToAnchor(container.bottomAnchor, constant = -16.0)
    } else {
        c += msgLabel.bottomAnchor.constraintEqualToAnchor(container.bottomAnchor, constant = -16.0)
    }
...
private fun makeColoredButton(title: String, color: UIColor, target: ActionTarget): UIButton {
    ...
    btn.contentEdgeInsets = UIEdgeInsetsMake(14.0, 28.0, 14.0, 28.0)
    btn.translatesAutoresizingMaskIntoConstraints = false
    btn.addTarget(target, NSSelectorFromString("invoke:"), forControlEvents = UIControlEventPrimaryActionTriggered)
    return btn
}
...
private fun buildDetailsTvos(issue: Issue): String {
...
private fun fmtTimeTvos(ms: Long): String {
