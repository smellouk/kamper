# kamper-migrate-kotlin

## Purpose

Guides a safe Kotlin version bump in Kamper — updates the single `kotlin` entry in `gradle/libs.versions.toml`, checks Kotlin↔Compose, Kotlin↔Coroutines, and Kotlin↔Mokkery compatibility before touching any file, and verifies the build passes before committing.

## When to Use

- User invokes `/kamper-migrate-kotlin <new-version>` (e.g. `/kamper-migrate-kotlin 2.4.0`).
...
2. **Check compatibility before editing** — verify `<new-version>` is supported by the current:
   ...
   - Minimum Gradle version required by the new KGP — see `/kamper-migrate-gradle` Reference
...
Kamper is already on K2 (Kotlin 2.3.21). K2-specific considerations for future bumps:
