# konitor-migrate-gradle

## Purpose

Guides a safe Gradle wrapper version bump in Konitor — updates `gradle/wrapper/gradle-wrapper.properties`, checks Gradle↔AGP and Gradle↔Kotlin compatibility before editing, runs the wrapper self-check, and verifies the build before committing.

## When to Use

- User invokes `/konitor-migrate-gradle <new-version>` (e.g. `/konitor-migrate-gradle 8.14`).
- When a new Gradle release is available.
- When a new AGP version requires a higher minimum Gradle (see `/konitor-migrate-agp` Reference).
...
Rule: never downgrade Gradle below the minimum required by the current AGP. If a new Gradle version is also paired with a new AGP bump, run `/konitor-migrate-gradle` first, then `/konitor-migrate-agp`.
