...
Scaffolds a complete Konitor KMP performance module skeleton — all 8 platform source sets, `build.gradle.kts` using the `konitor.kmp.library` convention plugin, Info/Config/Watcher/Performance class stubs with INVALID sentinel and Builder/DEFAULT pattern, `expect`/`actual` Module.kt declarations, commonTest stubs, and the `settings.gradle.kts` registration line — leaving the new module ready to compile.

## When to Use

- User invokes `/konitor-new-module <name>` where `<name>` is the module name in lowercase letters and digits (e.g. `thermal2`, `battery`, `gpu`).
...
3. **Create the directory tree** under `libs/modules/{name}/src/`. Full structure:
   ...
   ├── build.gradle.kts
   └── src/
       ├── commonMain/kotlin/com/smellouk/konitor/{name}/
       │   ├── {Name}Info.kt
       │   ├── {Name}Config.kt
       │   ├── {Name}Watcher.kt
       │   ├── {Name}Performance.kt
       ...
       ├── commonTest/kotlin/com/smellouk/konitor/{name}/
       │   ├── {Name}ConfigBuilderTest.kt
       │   └── repository/
       │       └── {Name}InfoMapperTest.kt
       ├── androidMain/kotlin/com/smellouk/konitor/{name}/
       │   ├── Module.kt                     (actual val {Name}Module)
       │   └── repository/
       │       ├── {Name}InfoRepositoryImpl.kt
       │       └── source/
       │           └── Android{Name}InfoSource.kt
       ├── androidTest/kotlin/com/smellouk/konitor/{name}/
       │   └── repository/
...
5. **Write `commonMain` class stubs** using the verbatim stubs from the Reference section:
   ...
   - `{Name}Config.kt` — data class implementing `Config`, with `@KonitorDslMarker Builder` and `DEFAULT`
...
9. **Update `settings.gradle.kts`** by inserting `include(":libs:modules:{name}")` in alphabetical order within the konitor modules block. If the line already exists, abort with a clear error.
...
Insert this line in alphabetical order within the konitor modules include block:
...
- **Builder/DEFAULT:** every Config has `@KonitorDslMarker object Builder { ... fun build() = ... }` and `companion object { val DEFAULT = Builder.build() }`
