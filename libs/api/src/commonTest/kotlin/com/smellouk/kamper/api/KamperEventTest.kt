...
class KamperEventTest {

    @Test
    fun `KamperEvent stores moduleName timestampMs platform and base Info`() {
    ...
    @Test
    fun `KamperEvent supports Info INVALID sentinel as info payload`() {
        val event = KamperEvent(
            moduleName = "cpu",
            timestampMs = 0L,
            platform = "android",
    ...
    @Test
    fun `KamperEvent equality is value-based for data class semantics`() {
        val info = object : Info {}
        val a = KamperEvent("cpu", 100L, "jvm", info)
        val b = KamperEvent("cpu", 100L, "jvm", info)
    ...
    fun `IntegrationModule is Cleanable so Engine can call clean on removal`() {
        var cleaned = false
        var receivedEvent: KamperEvent? = null
        ...
        // onEvent receives a typed KamperEvent
        val event = KamperEvent("cpu", 1L, "jvm", Info.INVALID)
