...
// tvOS overlay actual.
// Differences vs appleMain/KamperUi.kt:
...
actual object KamperUi {
    ...
    // Public facade — used by react-native-kamper iOS/tvOS TurboModule (Phase 14 output).
    fun show() = attach()
...
private class TvosChipViewController(
    ...
    private val repo: KamperUiRepository
    ...
    override fun pressesBegan(presses: Set<*>, withEvent: UIPressesEvent?) {
        for (press in presses.filterIsInstance<UIPress>()) {
            when (press.type) {
                UIPressTypeSelect -> {
                    when (KamperUi.chipState) {
                        ChipState.PEEK -> KamperUi.expandChip()                       // D-02 first Select
                        ChipState.EXPANDED -> KamperUi.openPanel(this, repo)          // D-02 second Select
                    }
                    return
                }
