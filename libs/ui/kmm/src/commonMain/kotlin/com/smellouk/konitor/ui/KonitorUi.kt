...
/**
 * KonitorUi — overlay control surface.
 *
 ...
 *   in `konitor/react-native/{android,ios}/`) — common code does NOT need to
 *   call `show()`.
 ...
 *   shared konitor-ui artifact can hide the overlay.
 *
 ...
 */
expect object KonitorUi {
    fun configure(block: KonitorUiConfig.() -> Unit)
    fun attach()
    fun detach()

    /**
     * Hide the overlay. Public facade for external consumers
     * (e.g. react-native-konitor TurboModule). Equivalent to detach().
     */
    fun hide()
}
