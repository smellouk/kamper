 ...
 *       types KamperPanel exposes (onSettingsChange, onStartEngine, etc.) are wired
 ...
 *       so any future regression in KamperPanel's parameter types breaks compilation.
 *
 * D-12: Non-rendering tab-routing — verifies the `when(selectedTab)` mapping
 *       in KamperPanel.kt routes the four tab indices to their tabs correctly.
 *       The test mirrors KamperPanel's routing logic; if KamperPanel's mapping
 *       changes, this test must change too — guarding against accidental regressions.
 */
@Suppress("IllegalIdentifier")
class KamperPanelTest {

    /** Mirror of KamperPanel.kt's `when(selectedTab)` routing block. */
    ...
    @Test
    fun `onSettingsChange shape is value_passing KamperUiSettings to Unit`() {
        var captured: KamperUiSettings? = null
        val cb: (KamperUiSettings) -> Unit = { captured = it }

        val sample = KamperUiSettings()
        cb(sample)
