...
class KonitorPanelActivity : AppCompatActivity() {
    ...
    private fun sharePerfettoTrace() {
        val repo = KonitorUi.repository ?: return
        if (repo.recordingSampleCount.value == 0) return
        val fileName = "konitor_${System.currentTimeMillis()}.perfetto-trace.gz"
        val file = File(cacheDir, fileName)
        FileOutputStream(file).use { fos -> repo.exportTraceToFile(fos) }
        val uri = FileProvider.getUriForFile(this, "$packageName.konitor.provider", file)
