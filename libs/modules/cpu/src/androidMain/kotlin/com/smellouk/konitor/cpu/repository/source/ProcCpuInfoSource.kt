...
internal class ProcCpuInfoSource(private val logger: Logger) : CpuInfoSource {
    ...
    override fun getCpuInfoDto(): CpuInfoDto {
        ...
        val statLine = try { ProcFileReader.getCpuProcStatTime().also { Log.d("Konitor/CPU/Proc", "/proc/stat line: $it") } }
            catch (e: Exception) { Log.e("Konitor/CPU/Proc", "/proc/stat read failed: ${e.javaClass.simpleName}: ${e.message}"); return CpuInfoDto.INVALID }
        val pidStatLine = try { ProcFileReader.getCpuProcPidStatTime(pid).also { Log.d("Konitor/CPU/Proc", "/proc/$pid/stat line: $it") } }
            catch (e: Exception) { Log.e("Konitor/CPU/Proc", "/proc/$pid/stat read failed: ${e.javaClass.simpleName}: ${e.message}"); return CpuInfoDto.INVALID }
        val currentCpuInfoDto = parse(statLine, pidStatLine)

        Log.d("Konitor/CPU/Proc", "parsed current=$currentCpuInfoDto cached=${if (this::cachedDto.isInitialized) cachedDto else null}")
        return if (!this::cachedDto.isInitialized) {
            ...
            CpuInfoDto(
                ...
                Log.d("Konitor/CPU/Proc", "delta dto=$it")
                cachedDto = currentCpuInfoDto
            }
        }
    }
