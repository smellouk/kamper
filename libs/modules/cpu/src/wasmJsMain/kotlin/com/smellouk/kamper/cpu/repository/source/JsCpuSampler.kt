...
@JsFun("""() => {
    if (window.__kamperCpu) return;
    window.__kamperCpu = { load: 0.0, running: false, handle: 0, expected: 0 };
}""")
private external fun jsCpuInit()

@JsFun("""() => {
    var s = window.__kamperCpu;
...
@JsFun("() => { var s = window.__kamperCpu; return s ? s.load : 0.0; }")
