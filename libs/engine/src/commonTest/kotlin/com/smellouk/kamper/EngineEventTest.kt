 ...
 * Plan 04 — Engine event API + buffer + KamperConfig.eventsEnabled.
 */
...
class EngineEventTest {
    ...
    private class RecordingIntegration : IntegrationModule {
        val events: MutableList<KamperEvent> = mutableListOf()
        override fun onEvent(event: KamperEvent) { events += event }
        override fun clean() = Unit
    }

    private fun engine(eventsEnabled: Boolean = true, logger: Logger = Logger.EMPTY): Engine =
        Engine().also { e ->
            e.config = KamperConfig.Builder().apply {
                this.logger = logger
    ...
    @Test
    fun logEvent_buffers_record_and_dispatches_KamperEvent() {
