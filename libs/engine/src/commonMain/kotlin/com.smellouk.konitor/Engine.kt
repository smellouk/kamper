...
import com.smellouk.konitor.api.currentPlatform
...
open class Engine {
    ...
    // Visible for testing
    @PublishedApi
    internal var config: KonitorConfig = KonitorConfig.DEFAULT
        set(value) {
            logger = value.logger
            field = value
        }
    ...
    inline fun <reified I : Info> emit(info: I) {
        (mapListeners[I::class] as? List<InfoListener<I>>)?.forEach { it.invoke(info) }
    }
    ...
    /**
     * Register an [IntegrationModule] that receives a [KonitorEvent] for every Info
     ...
     *
     * Returns `this` to support fluent chaining: `konitor.addIntegration(a).addIntegration(b)`.
     */
    ...
    @PublishedApi
    internal fun dispatchToIntegrations(event: KonitorEvent) {
        ...
        // a single integration's failure must never affect the others or Konitor core.
     ...
     * `KonitorEvent(moduleName="event")` to integrations.
     *
     * No-op when [KonitorConfig.eventsEnabled] is `false` (D-06).
     */
     ...
     * and dispatches a `KonitorEvent` carrying the duration in milliseconds.
     *
    ...
    fun dumpEvents() {
        ...
        sb.append("===> Konitor Events: begin\n")
        snapshot.forEach { e ->
    ...
    inline fun <C : Config, reified I : Info> install(module: PerformanceModule<C, I>) {
        with(module) {
            ...
            // has dispatched the KonitorEvent. The lowercase moduleName mirrors the
