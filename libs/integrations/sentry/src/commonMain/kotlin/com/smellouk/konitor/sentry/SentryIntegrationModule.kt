...
/**
 * Forwards Konitor events to Sentry.
 *
 ...
 * depend on [com.smellouk.konitor.cpu.CpuInfo] / [com.smellouk.konitor.memory.MemoryInfo] /
 * [com.smellouk.konitor.fps.FpsInfo] — that would make konitor-sentry transitively pull in
...
public class SentryIntegrationModule internal constructor(
    ...
    init {
        try {
            ...
            // Konitor has no logger handle here — the SDK's own error path (if any) is
    ...
    override fun onEvent(event: KonitorEvent) {
        try {
            ...
            when (event.moduleName) {
                ...
                "cpu"   -> handleCpuMetric(event, threshold = config.forwardCpuAbove, category = "konitor.cpu")
                "memory" -> handleMemoryMetric(event, threshold = config.forwardMemoryAbove, category = "konitor.memory")
                "fps"   -> if (config.forwardFps) handleBreadcrumb(event, category = "konitor.fps", level = SentryLevel.INFO)
    ...
    private fun handleIssue(event: KonitorEvent) {
        val message = "Konitor issue from ${event.moduleName} on ${event.platform}: ${event.info}"
        Sentry.captureException(RuntimeException(message))
    }

    private fun handleCpuMetric(event: KonitorEvent, threshold: Float?, category: String) {
    ...
    private fun handleMemoryMetric(event: KonitorEvent, threshold: Float?, category: String) {
        if (threshold == null) return
     ...
     */
    private fun handleUserEvent(event: KonitorEvent) {
