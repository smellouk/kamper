...
@JsFun("""() => {
    if (window.__kamperFps) return;
    window.__kamperFps = { running: false, handle: 0, count: 0, startMs: 0.0, lastMs: 0.0 };
}""")
private external fun jsFpsInit()

@JsFun("""() => {
    var s = window.__kamperFps;
...
@JsFun("() => { var s = window.__kamperFps; return s ? s.count : 0; }")
private external fun jsFpsGetCount(): Int

@JsFun("() => { var s = window.__kamperFps; return s ? s.startMs : 0.0; }")
private external fun jsFpsGetStartMs(): Double

@JsFun("() => { var s = window.__kamperFps; return s ? s.lastMs : 0.0; }")
private external fun jsFpsGetLastMs(): Double

@JsFun("() => { var s = window.__kamperFps; if (s) { s.count = 0; s.startMs = 0.0; s.lastMs = 0.0; } }")
