...
/**
 * Convention plugin that bundles the Konitor KMP library boilerplate that is otherwise
 * repeated identically across all 8 publishing modules (engine + cpu/fps/memory/network/jank/gc/thermal/issues)
 * plus the api module.
 *
 * Applied as: id("konitor.kmp.library") in the module's plugins{} block.
 *
 * Behavior:
 ...
 *        - com.android.library
 *        - dev.mokkery
 *   2. Applies AndroidConfigPlugin so consumers don't need a separate id("konitor.android.config").
 *   3. Configures the KotlinMultiplatformExtension with all 10 Konitor targets:
 ...
 *   - The id("konitor.publish") plugin — only publishing modules apply it.
 *
 * Modules that CANNOT use this plugin (Critical Finding #2):
 *   - libs/xcframework — applies only org.jetbrains.kotlin.multiplatform; no com.android.library, no mokkery.
 *   - libs/ui/android — uses only 3 targets and Compose plugins.
